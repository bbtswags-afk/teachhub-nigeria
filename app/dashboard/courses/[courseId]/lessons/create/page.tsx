...
import { ArrowLeft, ArrowRight, Save, Upload, Video, FileText, HelpCircle, Layers, Plus, Trash, Image as ImageIcon, Check, Gamepad2, AlignLeft, GripVertical, X, Move, ImagePlus } from "lucide-react";
...
export default function CreateLessonPage() {
    ...
    const [formData, setFormData] = useState<FormData>({
        ...
        summary: "",
        lessonPlan: "",
        contentBlocks: [{ id: "start-1", type: "text", content: "" }],
        gameUrl: "",
        quizzes: [{ question: "", options: ["", "", "", ""], correctOption: 0 }],
        flashcards: [{ front: "", back: "", color: "bg-blue-100" }]
    });
    ...
    const handleSubmit = async () => {
        ...
        setLoading(true);
        try {
            // Extract media for the legacy relation
            ...
            const cleanData = {
                ...
                quizzes: formData.quizzes.filter(q => q.question.trim().length > 0),
                // Filter out incomplete flashcards
                flashcards: formData.flashcards.filter(f => f.front.trim().length > 0 && f.back.trim().length > 0)
            };

            const result = await createLesson(params.courseId as string, cleanData);
        ...
        return (
            <div className="min-h-screen flex items-center justify-center bg-slate-50">
                <div className="bg-white p-12 rounded-3xl shadow-xl text-center max-w-lg mx-auto">
                    ...
                    <p className="text-slate-500 mb-8 text-lg">Lesson Published Successfully.</p>
                    <button
                        ...
                        {formData.quizzes.length > 0 && (
                            <div className="pt-8 border-t border-slate-100">
                                <div className="flex items-center justify-between mb-8">
                                    ...
                                    <button onClick={() => setStep(5)} className="text-primary text-xs font-bold hover:underline">Edit Quiz</button>
                                </div>
                        ...
                        {formData.flashcards.length > 0 && (
                            <div className="pt-8 border-t border-slate-100">
                                <div className="flex items-center justify-between mb-8">
                                    ...
                                    <button onClick={() => setStep(6)} className="text-primary text-xs font-bold hover:underline">Edit Flashcards</button>
                                </div>
                                <FlashcardDeck cards={formData.flashcards} />
                            </div>
                        )}

                        {/* Game Embed (Moved to Bottom) */}
                        {formData.gameUrl && (
                            <div className="pt-8 border-t border-slate-100">
                                <div className="flex items-center justify-between mb-4">
                                    <h4 className="font-bold text-slate-900 flex items-center gap-2"><Gamepad2 size={20} className="text-primary" /> Game Time!</h4>
                                    <button onClick={() => setStep(4)} className="text-primary text-xs font-bold hover:underline">Edit Game</button>
                                </div>
    ...
    return (
        <div className="max-w-5xl mx-auto p-6 pb-20">
            ...
            >
                ...
                {step === 2 && renderStep2()}
                {step === 3 && renderStep3()}
                {step === 4 && renderStep5()} {/* Game */}
                {step === 5 && renderStep6()} {/* Quiz */}
                {step === 6 && renderStep7()} {/* Flashcards */}
                {step === 7 && renderStep8()} {/* Review */}
            </motion.div>
            ...
            <div className="fixed bottom-0 left-0 w-full bg-white border-t border-slate-200 p-4 z-50">
                <div className="max-w-5xl mx-auto flex justify-between items-center">
                    ...
                    <div className="flex items-center gap-2">
                        ...
                        >
                            {step === steps.length ? (loading ? "Publishing..." : "Publish Lesson") : "Next Step"}
